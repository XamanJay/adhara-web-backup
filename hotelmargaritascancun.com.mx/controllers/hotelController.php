...
class hotelController{
	...
	function getPromocion($llegada){
		try {

			$time = strtotime('04/12/2020');
			$limite = date('Y-m-d',$time);

			if(strtotime($llegada) < strtotime($limite)){
				$today = date("Y-m-d");
				$db = new db();
				$conn = $db->connection();
				$query = "SELECT * FROM  promociones  WHERE isDeleted = 0 AND '$today' BETWEEN fecha_inicio AND fecha_termina AND hotel_id=1;";
				$stmt = $conn->prepare($query);
				$stmt->execute();
				$count = $stmt->rowCount();
				if ($count > 0) {
	...
	function getRooms($seasons,$fechaEntrada,$fechaSalida,$idHotel){
		...
		try {
			...
			foreach ($seasons as $season) {
				...
				$stmt = $conn->prepare($query);
				$stmt->bindParam(1,$idHotel);
				$stmt->bindParam(2,$season->getIdRoom());
				$stmt->execute();
				$count = $stmt->rowCount();
				if ($count > 0) {
	...
	function getRoom($seasons,$fechaEntrada,$fechaSalida,$idHotel){
		...
		try {
			...
			$stmt = $conn->prepare($query);
			$stmt->bindParam(1,$idHotel);
			$stmt->bindParam(2,$seasons->getIdRoom());
			$stmt->execute();
			$count = $stmt->rowCount();
			if ($count > 0) {
	...
	function getKidsAllow($cuarto,$kids){
		try {
	...
	function getStopSales($cuarto){
		...
		try {
			...
			while ($contNight < $cuarto->getNoches()) {
				...
				$stmt = $conn->prepare($query);
				$stmt->bindParam(1,$cuarto->getIdr());
	...
	function getPagoEnDestino($cuarto){
		...
		try {
			...
			$stmt = $conn->prepare($query);
			$stmt->bindParam(1,$cuarto->getFechaEntrada());
			$stmt->bindParam(2,$cuarto->getFechaSalida());
			$stmt->execute();
			$count = $stmt->rowCount();
			if ($count > 0) {
	...
	function getCapacity($cuarto,$adultos,$niños){
	...
	function getTarifas($cuarto,$adults,$kids){
		...
		try {
			...
			if($cuarto->getNoches() == 1){
				...
				$stmt = $conn->prepare($query);
				$stmt->bindParam(1,$cuarto->getIdh());
				$stmt->bindParam(2,$cuarto->getIdr());
				$stmt->execute();
				$count = $stmt->rowCount();
				if ($count > 0) {
	...
	function getTarifasPromo($cuarto,$adults,$kids){
	...
	function getPrecioClubestrella($amount){
		try {
