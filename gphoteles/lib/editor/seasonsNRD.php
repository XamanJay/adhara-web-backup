...
Editor::inst( $db, 'seasons','id' )
    ...
    ->where( 'idRoom', 12 ) // Habitacion estandar con alimentos
    ->where( 'isDeleted', 0)
    ->on( 'preEdit', function ( $editor, $id, $values ) {
        $editor
            ->field( 'single' )
            ->setValue(($values['single']+8)+(($values['single']+8)*0.19));
          } )
    ->on( 'preEdit', function ( $editor, $id, $values ) {
        $editor
            ->field( 'doble' )
            ->setValue(($values['single']+(2*8))+(($values['single']+(2*8))*0.19));
          } )
    ->on( 'preEdit', function ( $editor, $id, $values ) {
        $editor
            ->field( 'triple' )
            ->setValue(($values['single']+$values['extra']+(3*8))+(($values['single']+$values['extra']+(3*8))*0.19));
          } )
    ->on( 'preEdit', function ( $editor, $id, $values ) {
        $editor
            ->field( 'cuadra' )
            ->setValue(($values['single']+(2*$values['extra'])+(4*8))+(($values['single']+(2*$values['extra'])+(4*8))*0.19));
          } )
    ->on('preEdit',function($editor,$id,$values){
        $editor
            ->field('mealAdults')
