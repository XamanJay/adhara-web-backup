...
Editor::inst( $db, 'seasons','id' )
    ...
    ->where( 'idRoom', 18 ) // Habitacion estandar con alimentos
    ->where( 'isDeleted', 0)
    ->on( 'preEdit', function ( $editor, $id, $values ) {
        $editor
            ->field( 'single' )
            ->setValue(($values['single']+$values['mealAdults']+10)+(($values['single']+$values['mealAdults']+10)*0.19));
          } )
    ->on( 'preEdit', function ( $editor, $id, $values ) {
        $editor
            ->field( 'doble' )
            ->setValue(($values['single']+(2*$values['mealAdults'])+10)+(($values['single']+(2*$values['mealAdults'])+10)*0.19));
          } )
    ->on( 'preEdit', function ( $editor, $id, $values ) {
        $editor
            ->field( 'triple' )
