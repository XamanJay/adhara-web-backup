...
Editor::inst( $db, 'seasons','id' )
    ...
    ->where( 'idRoom', 17 ) // Habitacion estandar con alimentos
    ->where( 'isDeleted', 0)
    ->on( 'preEdit', function ( $editor, $id, $values ) {
        $editor
            ->field( 'single' )
            ->setValue(($values['single']+$values['mealAdults']+5)+(($values['single']+$values['mealAdults']+5)*0.19));
          } )
    ->on( 'preEdit', function ( $editor, $id, $values ) {
        $editor
            ->field( 'doble' )
            ->setValue(($values['single']+(2*$values['mealAdults'])+5)+(($values['single']+(2*$values['mealAdults'])+5)*0.19));
          } )
    ->on( 'preEdit', function ( $editor, $id, $values ) {
        $editor
            ->field( 'triple' )
