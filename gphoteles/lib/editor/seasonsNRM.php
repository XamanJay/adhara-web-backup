...
Editor::inst( $db, 'seasons','id' )
    ...
    ->where( 'idRoom', 11 ) // Habitacion estandar no Reembolsable
    ->where( 'isDeleted', 0)
    ->on( 'preEdit', function ( $editor, $id, $values ) {
        $editor
            ->field( 'single' )
            ->setValue(($values['single']+($values['single']*0.19))*0.90);
          } )
    ->on( 'preEdit', function ( $editor, $id, $values ) {
        $editor
            ->field( 'doble' )
    ...
    ->on('preEdit',function($editor, $id,$values){
        $editor
            ->field('triple')
            ->setValue((($values['single']+$values['extra'])+(($values['single']+$values['extra'])*0.19))*0.90);
        })
    ->on('preEdit',function($editor, $id,$values){
        $editor
            ->field('cuadra')
            ->setValue((($values['single']+(2*$values['extra']))+(($values['single']+(2*$values['extra']))*0.19))*0.90);
        })
    ->on('preEdit',function($editor,$id,$values){
        $editor
            ->field('extra')
