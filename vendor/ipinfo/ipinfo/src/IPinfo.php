...
class IPinfo
{
    ...
    public function __construct($access_token = null, $settings = [])
    {
        ...
        $countries_file = ($settings['countries_file']) ? $settings['countries_file'] : self::COUNTRIES_FILE_DEFAULT;
        $this->countries = $this->readCountryNames($countries_file);

        if (array_key_exists('cache', $settings)) {
          $this->cache = $settings['cache'];
        } else {
          $maxsize = ($settings['cache_maxsize']) ? $settings['cache_maxsize'] : self::CACHE_MAXSIZE;
          $ttl = ($settings['cache_ttl']) ? $settings['cache_ttl'] : self::CACHE_TTL;
    ...
    {
        $response_details = $this->getRequestDetails($ip_address);
    ...
    {
      $country = ($details['country']) ? $details['country'] : null;
      $details['country_name'] = ($this->countries[$country]) ? $this->countries[$country] : null;
     ...
     * @throws IPinfoException
     */
    public function getRequestDetails($ip_address)
    {
